...
class SettleResults:
    ...
    def merge_pkl(self, num_epoch, type_list, start_epoch=0):
        # 修改类型列表，改为使用我们的新命名
        if 'train_val_score' in type_list:
            type_list.remove('train_val_score')
            if 'val_score' not in type_list:
                type_list.append('val_score')        
    ...
    def get_final_score(self):
        """获取最终阶段的测试分数"""
        # 首先尝试查找最佳模型评估保存的分数文件
        best_score_file = os.path.join(self.exp_dir, 'best_models', 'best_stage2_test_score.pkl')
        if os.path.exists(best_score_file):
            with open(best_score_file, 'rb') as f:
                score_pred = pickle.load(f)
            score_pred_df = pd.DataFrame(score_pred)
        ...
        # 如果找不到最佳模型评估文件，尝试旧版的文件查找方式
        file = [kk for kk in os.listdir(os.path.join(self.exp_dir, 'epoch'))
                if re.search('allepo_\d+_test_score.pkl', kk) is not None or 
                   re.search('epoch\d+_test_score.pkl', kk) is not None]

        if not file:
            print(f"警告: 在 {os.path.join(self.exp_dir, 'epoch')} 中找不到测试分数文件")
            return None

        # 选择找到的最后一个文件
        latest_file = sorted(file)[-1]
        print(f"使用测试分数文件: {latest_file}")

        with open(os.path.join(self.exp_dir, 'epoch', latest_file), 'rb') as f:
            if 'allepo' in latest_file:
                score_pred_all = pickle.load(f)
                max_epoch = max(score_pred_all.keys())
                score_pred = score_pred_all[max_epoch]
            else:
                score_pred = pickle.load(f)
